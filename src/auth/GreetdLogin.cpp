...
static int socketConnect() {
    ...
    if (FD < 0) {
        Log::logger->log(Log::ERR, "Failed to create socket");
        return -1;
    }
    ...
    if (connect(FD, (sockaddr*)&serverAddress, SUN_LEN(&serverAddress)) < 0) {
        Log::logger->log(Log::ERR, "Failed to connect to greetd socket");
        return -1;
    }
...
static int sendToSock(int fd, const std::string& PAYLOAD) {
    ...
    while (wrote < sizeof(LEN)) {
        ...
        if (n < 1) {
            Log::logger->log(Log::ERR, "Failed to write to greetd socket");
            return -1;
        }
...
static std::string readFromSock(int fd) {
    ...
    while (numRead < sizeof(len)) {
        ...
        if (n < 1) {
            Log::logger->log(Log::ERR, "Failed to read from greetd socket");
            return "";
        }
...
static bool sendGreetdRequest(int fd, const VGreetdRequest& request) {
    if (fd < 0) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Invalid socket fd");
        return false;
    }
    ...
    if (!GLZRESULT.has_value()) {
        ...
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to serialize request: {}", GLZERRORSTR);
        return false;
    }

    if (std::holds_alternative<SGreetdPostAuthMessageResponse>(request))
        Log::logger->log(Log::TRACE, "[GreetdLogin] Request: REDACTED");
    else
        Log::logger->log(Log::TRACE, "[GreetdLogin] Request: {}", GLZRESULT.value());

    if (sendToSock(fd, GLZRESULT.value()) < 0) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to send payload to greetd");
        return false;
    }
...
void CGreetdLogin::init() {
    ...
    if (m_loginUserName.empty()) {
        Log::logger->log(Log::CRIT, "[GreetdLogin] No user specified");
        m_ok = false;
        return;
    } else
        Log::logger->log(Log::INFO, "[GreetdLogin] Login user: {}", m_loginUserName);

    m_thread = std::thread([this]() {
        m_socketFD = socketConnect();
        if (m_socketFD < 0) {
            Log::logger->log(Log::ERR, "[GreetdLogin] Failed to connect to greetd socket");
            m_ok = false;
        }
...
std::expected<VGreetdResponse, eRequestError> CGreetdLogin::request(const VGreetdRequest& req) {
    ...
    if (RESPONSESTR.empty()) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to read response from greetd");
        m_ok = false;
        return std::unexpected(GREETD_REQUEST_ERROR_READ);
    }

    Log::logger->log(Log::TRACE, "[GreetdLogin] Response: {}", RESPONSESTR);
    ...
    if (!GLZRESULT.has_value()) {
        ...
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to parse response from greetd: {}", GLZERRORSTR);
        m_ok = false;
...
void CGreetdLogin::handleResponse(const VGreetdRequest& request, const VGreetdResponse& response) {
    if (std::holds_alternative<SGreetdErrorResponse>(response)) {
        ...
        Log::logger->log(Log::ERR, "[GreetdLogin] Request failed: {} - {}", (int)m_state.errorType, m_state.error);
        ...
        Log::logger->log(Log::INFO, "[GreetdLogin] Auth message: {} - {}", (int)m_state.authMessageType, m_state.message);
        ...
        Log::logger->log(Log::ERR, "Unknown response from greetd");
}
...
void CGreetdLogin::cancelSession() {
    ...
    if (!RESPONSE.has_value()) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to cancel session: {}", getErrorString(RESPONSE.error()));
        return;
    }
...
void CGreetdLogin::createSession() {
    if (m_state.authMessageType != GREETD_INITIAL && m_state.errorType != GREETD_ERROR_AUTH)
        Log::logger->log(Log::WARN, "[GreetdLogin] Trying to create a session, but last one still active?");

    Log::logger->log(Log::INFO, "[GreetdLogin] Creating session for user {}", m_loginUserName);
    ...
    if (!RESPONSE.has_value()) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to create session: {}", getErrorString(RESPONSE.error()));
        return;
    }
...
void CGreetdLogin::processInput() {
    ...
    while (m_ok && (m_state.authMessageType == GREETD_AUTH_INFO || m_state.authMessageType == GREETD_AUTH_ERROR)) {
        ...
        if (!RESPONSE.has_value()) {
            Log::logger->log(Log::ERR, "[GreetdLogin] No response after empty response: {}", getErrorString(RESPONSE.error()));
            return;
        }
    ...
    if (m_state.errorType != GREETD_OK || (m_state.authMessageType != GREETD_AUTH_SECRET && m_state.authMessageType != GREETD_AUTH_VISIBLE)) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Didn't successfully get a promt from greetd!");
    ...
    if (!RESPONSE.has_value()) {
        Log::logger->log(Log::ERR, "[GreetdLogin] Failed to send auth response: {}", getErrorString(RESPONSE.error()));
