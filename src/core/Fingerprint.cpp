...
static void fingerprintCheckCallback(std::shared_ptr<CTimer> self, void* data) {
    if (g_pFingerprint->isAuthenticated())
        g_pHyprlock->unlock();
}

std::optional<std::thread> CFingerprint::start() {
    if (!m_bEnabled)
        return {};
    return std::thread([this]() {
        ...
        if (m_sDBUSState.sleeping || startVerify())
            m_sDBUSState.connection->enterEventLoop();

        if (!m_sDBUSState.abort)
            releaseDevice();

        g_pHyprlock->addTimer(std::chrono::milliseconds(1), fingerprintCheckCallback, nullptr);
    });
}
